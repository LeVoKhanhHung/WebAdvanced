...
public class UserDAO {
    ...
    public boolean register(User user) {
        try {
            int insertedRecord = DbConnector.get().withHandle(h ->
                    h.createUpdate("INSERT INTO user(username, password, email, id_khachhang,active,public_key) values(?,?,?,?,?,?)")
    ...
    public boolean setPublicKey(String key, String email) {
        int rowAffected = DbConnector.get().withHandle(h ->
                h.createUpdate("UPDATE user SET public_key=? where email = ?")
                        .bind(0, key)
                        .bind(1, email)
                        .execute()
        );
